...
| Projet | Rôle |
| --- | --- |
| `Photobooth.Core` | Logique pure : interfaces, workflow « acteur » (états Idle/Capturing/Recording/Degraded), options, modèles GoPro JSON, retry borné, télémétrie diagnostic (`BoothTelemetry` : dernière raison d'échec d'impression). Aucune dépendance UI/hardware. |
