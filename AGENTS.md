...
**Lancer en local sur macOS (dev)** : `dotnet run --project src/Photobooth.App` (mode démo : Espace/Entrée = photo, V = vidéo, P = imprimer ; GoPro simulée via `PHOTOBOOTH_Gopro__Mode=fake`). ⚠️ **À lancer depuis un terminal de la session graphique** (iTerm/Terminal ouvert sur le bureau). Lancé depuis un process **détaché de la session Aqua** (SSH, daemon launchd, ou sous-process d'un agent/outil comme Claude Code), le backend `Avalonia.Native` ne peut pas créer son render timer (CVDisplayLink/CoreVideo) et l'app échoue **au démarrage** — avant tout code applicatif, donc sans rapport avec une régression — avec : `System.InvalidOperationException: Avalonia.Native was not able to start the RenderTimer. Native error code is: -6661`. `dotnet test` n'est **pas** affecté (les tests n'initialisent jamais la couche graphique). Il n'y a pas d'équivalent headless simple pour `Avalonia.Native` côté Mac : pour une **vérif visuelle automatisée**, passer par le Pi (`--drm`/`--fbdev`) ou un job CI Linux. La capture d'écran de vérif (`--screenshot <png>`, voir `README_NET8.md`) ne fonctionne donc en local que dans une session graphique.
